...
/*
 * display a cusotm dialog for our BankingAppGUi
 */
public class BankingAppDialog extends JDialog implements ActionListener{
    ...
    public BankingAppDialog(BankingAppGui bankingAppGui, User user){
        ...
        //put focus to the dialog so nothing else can be 
        //interacted with unitll it is close\
        setModal(true);

        
        //loads in the center of our bkaing gui
        setLocationRelativeTo(bankingAppGui);

        setDefaultCloseOperation(DISPOSE_ON_CLOSE);

        //prevent dialogt from being resized
        setResizable(false);

        //allows us to manually specify the size and position of each componet
        setLayout(null);

        //reference to our gui to update the current balance
        this.bankingAppGui = bankingAppGui;

        //regerence to the user info to make updates or retrieve their data form the db
        this.user=user;
    }
    ...
    public void addCurrentBalanceAndAmount(){
        ...
        enterAmountField.setBounds(15,80,getWidth() - 20 , 20);
    ...
    // a new jbutton whose text describews the action it does
    ...
    // speaks to the reciepient of the transfer
    ...
    public void addPastTransactionComponents(){
        ...
        pastTransactions = Connector.getPastTransaction(user);
     ...
     private void handleTransaction(String transactionType, float amountVal){
        ...
        if(transactionType.equalsIgnoreCase("Deposit")){
            ...
            transaction = new Transaction(user.getCid(), transactionType, new BigDecimal(amountVal), null);
        }else{
        ...
        // update database
        if(Connector.addTransactionToDatabase(transaction) && Connector.updateCurrentBalance(user)){
            // show success dialog
    ...
    private void handleTransfer(User user, String transferredUser, float amount){
        ...
        if(Connector.transfer(user, transferredUser, amount)){
            // show success dialog
